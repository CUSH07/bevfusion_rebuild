...
from ..utils.swin_utils import swin_converter
from ..utils.swin_utils import PatchEmbed, PatchMerging
from ..utils.swin_utils import FFN, DropPath, to_2tuple, trunc_normal_, trunc_normal_init, constant_init

#Swin Transformer 的核心，用于实现多头自注意力机制。它们通过划分图像为不同的窗口并在这些窗口内进行自注意力计算，有效地捕获局部特征。
...
class ShiftWindowMSA(nn.Module):
    ...
    #将原始特征图划分为多个小窗口，每个窗口包含了特征图的一部分，这些窗口将用于后续的自注意力计算
    #
    def window_partition(self, x):
        ...
        #输入特征图 x 的维度为 (B, H, W, C)，其中 B 是批次大小，H 和 W 分别是特征图的高度和宽度，C 是通道数。
        B, H, W, C = x.shape
        window_size = self.window_size
        
        #将输入特征图 x 按照 window_size（窗口大小）进行划分。H // window_size 和 W // window_size 计算每个维度上的窗口数。
        x = x.view(B, H // window_size, window_size, W // window_size,
                   window_size, C)
        #使用 permute 和 view 方法重新排列和调整张量的形状，得到划分后的窗口。
        windows = x.permute(0, 1, 3, 2, 4, 5).contiguous()

        #输出的 windows 的形状为 (num_windows*B, window_size, window_size, C)，
        #其中 num_windows 是每个批次中的窗口数，window_size 是窗口的大小，C 是通道数。
        windows = windows.view(-1, window_size, window_size, C)
        return windows

#构成了 Transformer 的层级结构。每个 SwinBlock 包含一个 ShiftWindowMSA 和一个前馈网络，
#SwinBlockSequence 则是将多个 SwinBlock 串联起来，形成一个网络阶段。
